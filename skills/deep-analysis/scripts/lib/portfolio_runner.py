...
def _render_html(portfolio_name: str, metrics: list[dict], health: dict, depth: str) -> str:
    ...
    for i, m in enumerate(ranked, 1):
        ...
        rows.append(
            ...
            f'  <td style="padding:10px 16px;color:var(--text-dim);font-family:-apple-system, SF Mono, Consolas, monospace,monospace;font-size:11px">#{i}</td>'
            f'  <td style="padding:10px 16px"><div style="font-weight:700;color:var(--text-bright)">{_esc(m["name"])}</div>'
            f'    <div style="font-size:11px;color:var(--text-dim);font-family:-apple-system, SF Mono, Consolas, monospace,monospace">{_esc(m["ticker"])} · {_esc(m.get("industry","—"))}</div></td>'
...
<div class="container" style="max-width:1180px;margin:0 auto;padding:30px 24px">
  ...
  <div style="text-align:center;margin:24px 0">
    <div style="font-family:-apple-system, "SF Pro Display", "PingFang SC", sans-serif,sans-serif;font-size:11px;letter-spacing:.24em;color:var(--neon-cyan);text-transform:uppercase">📊 PORTFOLIO HEALTH</div>
    <h1 style="font-size:32px;font-weight:900;color:var(--text-bright);margin:8px 0 4px">{portfolio_name}</h1>
    <div style="font-size:12px;color:var(--text-dim);font-family:-apple-system, SF Mono, Consolas, monospace,monospace">{health["n_valid"]}/{health["n_total"]} 只成分股 · {health["n_industries"]} 个行业</div>
  </div>
